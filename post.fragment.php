...
@keyframes iGuider-highlight {
	0% {
		box-shadow:0 0 20px -10px #fc880c;
	}
	50% {
		box-shadow:0 0 40px 0 #fc880c;
	}
	100% {
		box-shadow:0 0 20px -10px #fc880c;
	}
...
$(window).on('load',function(){
	...
	function hello(){
		...
		iGuider({
			...
			tourTitle:'lang',
			lang:{
				introTitle:'Let’s Schedule Posts!',
				introContent:'Can I help you and show you how you can use IGBooster to schedule your posts?',
			...
			steps:[{
				title:'Post', 
				content:'Use this button in future to add more posts', 
				target:'.nav-menu div > ul:nth-child(1) > li:nth-child(1) > a > span:nth-child(3)'
			},{ 
				title:'Schedule Photo and Videos',				
				content:'Select this tab to schedule or directly post a Photo or a Video',	
				target:'.types.clearfix label:nth-child(1) > div'
			},{ 
				title:'Schedule an Album',				
				content:'With the Album you can post slide/multi picture posts',	
				target:'.types.clearfix label:nth-child(3) > div'
			},{ 
				title:'Schedule Story',				
				content:'With IGBoosters you’re able to post and schedule your story as well, select this tab to do it!',	
				target:'.types.clearfix label:nth-child(2) > div'
			},{ 
				title:'Upload your Photo/Video',				
				content:'Upload your media from the sources available here.',	
				target:'.section-actions.clearfix'
			},{ 
				title:'Select Account',				
				content:'Select here the account you want to post on, if you have multiple accounts linked you can switch between them easily.',	
				target:'.dropdown select'
			},{ 
				title:'Media to post',				
				content:'Simply Drag in here the media you want to post',	
				target:'.drophere'
			},{ 
				title:'Post Caption',				
				content:'Select this tab to enter your post caption below',	
				target:'.tabheads.clearfix a:nth-child(1)'
			},{ 
				title:'First comment',				
				content:' Select this tab to enter your first comment below, for example for hashtags.',	
				target:'.tabheads.clearfix a:nth-child(2)'
			},{ 
				title:'Schedule this post?',				
				content:'If you want to schedule your post then tick this box, if you don’t you can post right away.',	
				target:'.section-content.controls div:nth-child(5) > label > span'
			},{ 
				title:'When do you want to post this?',				
				content:' If you ticked the “Schedule” box above, then define the posting date and time here.',	
				target:'.pos-r.mb-20 input'
			},{ 
				title:'Preview of your Post',				
				content:'See here how your post will appear after you post it.',	
				target:'.placeholder'
			},{ 
				title:'Post Now/Schedule Post',				
				content:'Click this button to Post Now or schedule your media and you’re done! :)',	
				target:'.fluid.large.button'
	
			}]
		});
		//return false;.finished-text
	}
	
	
	 $('.renewtop.button.helptour').on('click',function(){
		localStorage.removeItem('ipost');
		//;debugger
	});
	
	//hello();
	if(!localStorage.getItem('ipost')){
		hello();
		localStorage.setItem('ipost',true);
	}
    ...
    <div class='skeleton' id="post">
        <form action="javascript:void(0)"
            ...
            <div class="container-1200">
                ...
                <?php Event::trigger("noticeboard.place_notice"); ?>
                <div class="row">
                    ...
                    <div class="clearfix">
						...
						<a class="renewtop button helptour" href="<?= APPURL."/post/" ?>">HELP</a>
    ...
    <div class="modal fade" id="tagUserModal" tabindex="-1" role="dialog" aria-labelledby="tagUserModal" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h2 class="modal-title">Tag People</h2>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <div class="tag-media__wrapper">
                        <div class="tag-media__preview">
                            <img class="tag-media__image">
                        </div>
                        <div class="suggestion-box__mover">
                            <div class="suggestion-box__wrapper">
                                <div class="suggestion-box__circle circle-wrapper">
                                    <button class="circle-button"></button>
                                </div>
                                <div class="suggestion-box suggestion-box__arrow--top">
                                    <div class="suggestion-box__input--group">
                                        <input class="suggestion-box__input" placeholder="Type any name" id="autocomplete" type="text">
                                        <div class="suggestion-box__input--close"></div>
                                    </div>
                                    <div class="default-suggestions"></div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary btn-sm delete-tag" data-dismiss="modal">Close</button>
                    <button type="button" class="btn btn-primary btn-sm save-tag" data-dismiss="modal">Save</button>
                </div>
            </div>
        </div>
    </div>
